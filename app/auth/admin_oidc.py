...
by the stable OIDC subject (via an ``auth_identities`` link). The verifier is resolved from the
``admin_oidc_guard`` container binding so a test can swap in a fake (no live Keycloak needed).
"""
...
async def resolve_admin(request) -> User:
    ...
    if admin_role not in _realm_roles(dict(principal.claims)):
    ...
    if user is None:
        user = await User.create(
            name=principal.claims.get("name", email),
            email=email,
