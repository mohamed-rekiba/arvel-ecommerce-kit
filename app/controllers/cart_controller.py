...
async def line_presentation(
    ...
    """variant id → (product name in the active locale, variant name, product thumb URL, product
    slug). Two queries + the media eager-load — shared by the cart serializer and order reads."""
    ...
    for vid, variant in variants.items():
        ...
        out[vid] = (product.translation.name, variant.name, thumb, product.slug)
    return out
    ...
    return CartOut(
        ...
        items=[
            CartLineOut(
                ...
                product_name=looks.get(i.product_variant_id, ("?", "?", None, None))[0],
                variant_name=looks.get(i.product_variant_id, ("?", "?", None, None))[1],
