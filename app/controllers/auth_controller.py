...
async def forgot_password(request: Request, data: ForgotPasswordIn) -> ForgotPasswordOut:
    ...
    return ForgotPasswordOut(
        ...
        reset_token=password_reset_token(user.id, secret),  # dev only — normally emailed
    )
...
async def reset_password(request: Request, data: ResetPasswordIn) -> MessageOut:
    ...
    if len(data.password) < 8:
        raise ValidationException({"password": ["The password must be at least 8 characters."]})
    secret = Config.get("app.key", "")
    user_id = verify_password_reset_token(data.token, secret)
    user = await User.find(user_id) if user_id is not None else None
    if user is None:
        raise ValidationException({"token": ["This password reset token is invalid or has expired."]})
    user.password = data.password  # the User model's `hashed` cast argon2-hashes on assignment
    await user.save()
