...
export const api = {
  async login(email: string, password: string): Promise<string> {
    const res = await request<{ token: string }>('POST', '/login', {
      email,
      password
    })
    token.set(res.token)
  ...
  categories: () =>
    ...
    >('GET', '/admin/categories?per_page=100'),
  adminCategories: () =>
    request<Paginated<AdminCategoryOut>>('GET', '/admin/categories?per_page=100'),
  createCategory: (body: CategoryIn) =>
    request<AdminCategoryOut>('POST', '/admin/categories', body),
  ...
  updateCoupon: (id: number, body: CouponUpdateIn) =>
    request<AdminCouponOut>('PATCH', `/admin/coupons/${id}`, body),
  adminDeals: () => request<AdminDealOut[]>('GET', '/admin/deals'),
  createDeal: (body: DealIn) => request<AdminDealOut>('POST', '/admin/deals', body),
  updateDeal: (id: number, body: DealUpdateIn) =>
    request<AdminDealOut>('PATCH', `/admin/deals/${id}`, body),
  ...
  updateBanner: (id: number, body: BannerUpdateIn) =>
    request<AdminBannerOut>('PATCH', `/admin/banners/${id}`, body),
  ...
  adminUsers: (q = '') =>
    request<Paginated<AdminUserOut>>('GET', `/admin/users?q=${encodeURIComponent(q)}`),
  ...
  adminReviews: (status = 'pending') =>
    request<AdminReviewOut[]>('GET', `/admin/reviews?status=${status}`),
  ...
  updateSettings: (values: Record<string, string>) =>
    request<{ values: SettingsOutValues }>('PATCH', '/admin/settings', {
      values
    }),
