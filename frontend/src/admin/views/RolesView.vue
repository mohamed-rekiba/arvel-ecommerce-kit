...
async function load() {
  ...
  try {
    ...
    status.value = 'error'
    notice.value =
      e instanceof ApiError && e.status === 403 ? t('roles.no_view') : t('roles.load_error')
  }
}
onMounted(load)
</script>
...
<template>
  <section class="page">
    ...
    <div v-if="status === 'loading'" class="muted">
      {{ t('common.loading') }}
    </div>
    <div v-else-if="status === 'ready'" class="roles">
      <article v-for="role in roles" :key="role.id" class="role">
          ...
          <Tag
            :value="
              role.permissions.length
                ? t('roles.perms', { n: role.permissions.length })
                : t('roles.bypass')
            "
        ...
        <ul v-if="role.permissions.length" class="perms">
          <li v-for="p in role.permissions" :key="p">
            <code>{{ p }}</code>
          </li>
        </ul>
