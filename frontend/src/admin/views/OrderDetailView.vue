...
<template>
  <section class="page">
    ...
    <template v-else>
      <header class="head">
        <div>
          <RouterLink class="back" to="/admin/orders"
            >{{ tr('common.back') }} {{ tr('nav.orders') }}</RouterLink
          >
          <h1>
            {{ tr('orders.order') }} #{{ order.id }}
            <Tag :value="tr(`order.${order.status}` as MK)" :severity="severity(order.status)" />
            <Tag
              :value="tr(`pay.${order.payment_method}` as MK)"
              :severity="order.payment_method === 'cod' ? 'warn' : 'info'"
            />
          </h1>
          <p class="sub">
      ...
      <div class="grid">
        ...
        <section class="card">
          ...
          <p v-if="order.payments.length === 0" class="muted">
            {{ tr('orders.no_payments') }}
          </p>
          <ul class="lines">
            <li v-for="p in order.payments" :key="p.id">
              <span
                ><code>{{ p.charge_id }}</code></span
              >
              <span>
                {{ formatPrice(p.amount_cents) }}
                <Tag
                  :value="tr(`payment.${p.status}` as MK)"
                  :severity="
                    p.status === 'succeeded'
                      ? 'success'
                      : p.status === 'failed'
      ...
      <section class="card">
        ...
        <p v-if="order.history.length === 0" class="muted">
          {{ tr('orders.no_history') }}
        </p>
        <ol class="history">
          <li v-for="(event, i) in order.history" :key="i">
            ...
            <span v-if="event.properties.from" class="muted">
              {{ event.properties.from }} {{ tr('common.fwd') }}
              {{ event.properties.to }}
            </span>
