...
const form = reactive({ product_id: null as number | null, percent_off: 10, hours: 24 });
const productOptions = computed(() =>
  products.value.map((p) => ({ id: p.id, label: name(p) })),
);
  ...
  <section class="page">
    ...
    <div class="panel">
      <DataTable :value="deals" :loading="loading" data-key="id" size="small" striped-rows>
        <template #empty><p class="empty">{{ t("deals.none") }}</p></template>
        <Column :header="t('products.product')">
          <template #body="{ data }"><span class="pname">{{ data.product_name }}</span></template>
        </Column>
        <Column :header="t('deals.percent')">
          <template #body="{ data }"><Tag :value="`−${data.percent_off}%`" severity="warn" /></template>
        </Column>
        ...
        <Column :header="t('common.status')" style="width: 8rem">
          <template #body="{ data }">
            <Tag
              :value="data.live ? t('deals.live') : data.active ? t('deals.scheduled') : t('deals.paused')"
    ...
    <Dialog v-model:visible="dialogOpen" modal :header="t('deals.new')" :style="{ width: '26rem' }">
      <form class="form" @submit.prevent="create">
        <label class="field"><span>{{ t("products.product") }}</span>
          <Select v-model="form.product_id" :options="productOptions" optionLabel="label" optionValue="id" filter />
        </label>
        <label class="field"><span>{{ t("deals.percent") }}</span>
          <InputNumber v-model="form.percent_off" :min="1" :max="90" suffix="%" />
        </label>
        <label class="field"><span>{{ t("deals.duration") }}</span>
          <InputNumber v-model="form.hours" :min="1" :max="720" :suffix="` ${t('deal.hrs')}`" />
        </label>
        <Button type="submit" :label="saving ? t('common.saving') : t('common.save')" :disabled="saving || form.product_id == null" />
      </form>
    </Dialog>
  </section>
</template>
