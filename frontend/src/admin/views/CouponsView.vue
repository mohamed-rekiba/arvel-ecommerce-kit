...
async function create() {
  ...
  try {
    ...
    Object.assign(form, { code: "", type: "percent", value: 10, announce: false });
    await load();
  } catch (e) {
    notice.value =
  ...
  <section class="page">
    ...
    <div class="panel">
      <DataTable :value="coupons" :loading="loading" data-key="id" size="small" striped-rows>
        <template #empty><p class="empty">{{ t("coupons.none") }}</p></template>
        <Column :header="t('coupons.code')">
          <template #body="{ data }"><code class="code">{{ data.code }}</code></template>
        </Column>
        <Column :header="t('coupons.value')" style="width: 7rem">
          <template #body="{ data }">
            <Tag :value="data.type === 'percent' ? `−${data.value}%` : `−$${(data.value / 100).toFixed(2)}`" severity="warn" />
          </template>
        </Column>
        <Column :header="t('coupons.uses')" style="width: 6rem">
          <template #body="{ data }">{{ data.uses }}{{ data.usage_limit ? ` / ${data.usage_limit}` : "" }}</template>
        </Column>
        <Column :header="t('coupons.active')" style="width: 6rem">
          <template #body="{ data }">
            <ToggleSwitch :modelValue="data.active" :aria-label="t('coupons.active')" @update:modelValue="patch(data, { active: !data.active })" />
          </template>
        </Column>
        <Column :header="t('coupons.announce')" style="width: 8rem">
          <template #body="{ data }">
            <ToggleSwitch :modelValue="data.announce" :aria-label="t('coupons.announce')" @update:modelValue="patch(data, { announce: !data.announce })" />
          </template>
        </Column>
      </DataTable>
    </div>

    <Dialog v-model:visible="dialogOpen" modal :header="t('coupons.new')" :style="{ width: '24rem' }">
      <form class="form" @submit.prevent="create">
        <label class="field"><span>{{ t("coupons.code") }}</span><InputText v-model.trim="form.code" required /></label>
        <div class="row">
          <label class="field"><span>{{ t("coupons.type") }}</span>
            <Select v-model="form.type" :options="typeOptions" optionLabel="label" optionValue="value" />
          </label>
          <label class="field"><span>{{ t("coupons.value") }}</span>
            <InputNumber v-model="form.value" :min="1" :max="form.type === 'percent' ? 100 : 100000" />
          </label>
        </div>
        <label class="field field--switch"><span>{{ t("coupons.announce") }}</span><ToggleSwitch v-model="form.announce" /></label>
        <Button type="submit" :label="saving ? t('common.saving') : t('common.save')" :disabled="saving || !form.code" />
      </form>
    </Dialog>
  </section>
</template>
...
.code { font-family: var(--font-mono); font-size: 12.5px; font-weight: 700; }
...
.field :deep(input), .field :deep(.p-select) { width: 100%; }
