...
<template>
  <section class="page">
    ...
    <div class="panel">
      <DataTable :value="rows" :loading="loading" paginator :rows="15" data-key="id" size="small" striped-rows>
        <template #empty><p class="empty">{{ t("newsletter.none") }}</p></template>
        <Column :header="t('newsletter.email')">
          <template #body="{ data }"><span class="mono">{{ data.email }}</span></template>
        </Column>
        <Column :header="t('newsletter.locale')" style="width: 7rem">
          <template #body="{ data }"><span class="loc">{{ data.locale }}</span></template>
        </Column>
...
.head { margin-bottom: 20px; display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; }
...
.loc { text-transform: uppercase; font-size: 11.5px; font-weight: 700; color: var(--text-muted); }
