...
async function load() {
  ...
  try {
    ...
    status.value = 'error'
    notice.value =
      e instanceof ApiError && e.status === 403 ? t('audit.no_view') : t('audit.load_error')
  }
}
...
<template>
  <section class="page">
    ...
    <div v-else class="panel">
      ...
      >
        <template #empty
          ><p class="empty">{{ t('audit.none') }}</p></template
        >
        <Column :header="t('audit.when')">
          <template #body="{ data }"
            ><span class="muted nowrap">{{ when(data.created_at) }}</span></template
          >
        </Column>
        ...
        <Column :header="t('audit.subject')">
          <template #body="{ data }"
            ><span class="muted">{{
              data.subject_type ? `${data.subject_type} #${data.subject_id}` : '—'
            }}</span></template
          >
        </Column>
        <Column :header="t('audit.user')">
          <template #body="{ data }"
            ><span class="muted">{{
              data.causer_id != null ? `#${data.causer_id}` : '—'
            }}</span></template
          >
        </Column>
      </DataTable>
