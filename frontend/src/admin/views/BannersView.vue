...
function openCreate() {
  ...
  Object.assign(form, { copy: { en: empty(), fr: empty(), ar: empty() }, cta_to: "/catalog", sort: 0, active: true });
  activeLoc.value = "en";
...
function openEdit(banner: AdminBanner) {
  ...
  for (const loc of LOCALES) {
    ...
    if (text) copy[loc] = { title: text.title, subtitle: text.subtitle ?? "", chip: text.chip ?? "", cta_label: text.cta_label ?? "" };
  }
  Object.assign(form, { copy, cta_to: banner.cta_to, sort: banner.sort, active: banner.active });
  activeLoc.value = "en";
  dialogOpen.value = true;
}

function payloadTranslations() {
  const out: Record<string, { title: string; subtitle?: string | null; chip?: string | null; cta_label?: string | null }> = {};
  for (const loc of LOCALES) {
    const c = form.copy[loc];
    if (c.title.trim()) {
      out[loc] = { title: c.title, subtitle: c.subtitle || null, chip: c.chip || null, cta_label: c.cta_label || null };
    }
  }
  return out;
}
...
async function save() {
  ...
  notice.value = null;
  try {
    const body = { translations: payloadTranslations(), cta_to: form.cta_to, sort: form.sort, active: form.active };
    ...
    notice.value =
      e instanceof ApiError ? Object.values(e.errors)[0]?.[0] ??  t("common.save_error") : t("common.save_error");
  } finally {
    saving.value = false;
  }
}
...
async function remove(banner: AdminBanner) {
  if (!window.confirm(t("banners.delete_confirm", { title: banner.translations.en?.title ?? "#" + banner.id }))) return;
  try {
  ...
  <section class="page">
    ...
    <div class="panel">
      <DataTable :value="banners" :loading="loading" data-key="id" size="small" striped-rows>
        <template #empty><p class="empty">{{ t("banners.none") }}</p></template>
        ...
        <Column :header="t('banners.slide')">
          <template #body="{ data }">
            ...
            <div class="muted">{{ data.cta_to }} · {{ Object.keys(data.translations).join(", ") }}</div>
          </template>
        </Column>
        ...
        <Column :header="t('common.status')" style="width: 7rem">
          <template #body="{ data }">
            <Tag :value="data.active ? t('banners.live') : t('deals.paused')" :severity="data.active ? 'success' : 'secondary'" />
          </template>
        </Column>
        <Column header="" style="width: 16rem">
          <template #body="{ data }">
            ...
            <Button size="small" outlined severity="secondary" :label="data.active ? t('deals.pause') : t('deals.resume')" @click="toggleActive(data)" />
            <Button size="small" text severity="danger" :label="t('common.delete')" @click="remove(data)" />
          </template>
        </Column>
      </DataTable>
    </div>
    ...
    >
      <form class="form" @submit.prevent="save">
        ...
        <label class="field"><span>{{ t("banners.title") }} ({{ activeLoc }})</span>
          <InputText v-model="form.copy[activeLoc].title" :dir="activeLoc === 'ar' ? 'rtl' : 'ltr'" :required="activeLoc === 'en'" />
        </label>
        <label class="field"><span>{{ t("banners.subtitle") }} ({{ activeLoc }})</span>
          <InputText v-model="form.copy[activeLoc].subtitle" :dir="activeLoc === 'ar' ? 'rtl' : 'ltr'" />
        </label>
        <div class="row">
          <label class="field"><span>{{ t("banners.chip") }} ({{ activeLoc }})</span>
            <InputText v-model="form.copy[activeLoc].chip" :dir="activeLoc === 'ar' ? 'rtl' : 'ltr'" />
          </label>
          <label class="field"><span>{{ t("banners.cta_label") }} ({{ activeLoc }})</span>
            <InputText v-model="form.copy[activeLoc].cta_label" :dir="activeLoc === 'ar' ? 'rtl' : 'ltr'" />
          </label>
        </div>
        <div class="row">
          <label class="field"><span>{{ t("banners.cta_to") }}</span>
            <InputText v-model="form.cta_to" dir="ltr" />
          </label>
          <label class="field"><span>{{ t("banners.sort") }}</span>
            <InputNumber v-model="form.sort" :min="0" :max="99" />
          </label>
        </div>
        <label class="field field--switch"><span>{{ t("banners.live") }}</span><ToggleSwitch v-model="form.active" /></label>
        <Button type="submit" :label="saving ? t('common.saving') : t('common.save')" :disabled="saving || !form.copy.en.title" />
      </form>
    </Dialog>
  </section>
</template>
...
<style scoped>
.eyebrow { font-size: 11px; text-transform: uppercase; letter-spacing: .16em; color: var(--accent-text); font-weight: 600; }
.head { display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; margin-bottom: 20px; }
.head h1 { font-family: var(--font-display); font-size: 26px; font-weight: 700; margin: 6px 0 2px; }
...
.thumb { width: 110px; aspect-ratio: 16 / 7; object-fit: cover; border-radius: var(--radius-sm); border: 1px solid var(--border); }
.empty { text-align: center; color: var(--text-subtle); padding: 24px 0; }
.upl { display: inline-flex; align-items: center; position: relative; overflow: hidden; border: 1px solid var(--border-2); border-radius: var(--radius-sm); padding: 5px 10px; font-size: 12px; cursor: pointer; margin-inline-end: 6px; }
.upl input { position: absolute; inset: 0; opacity: 0; cursor: pointer; }
.locs { display: inline-flex; gap: 4px; margin-bottom: var(--space-4); border: 1px solid var(--border); border-radius: var(--radius-md); padding: 2px; }
.locs__tab { border: 0; background: none; font: inherit; padding: 4px 12px; border-radius: var(--radius-sm); cursor: pointer; color: var(--text-muted); }
.locs__tab.on { background: var(--text); color: var(--bg); }
.locs__todo { color: var(--warn); font-weight: 700; }
