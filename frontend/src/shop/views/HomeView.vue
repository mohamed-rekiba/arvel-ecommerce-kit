...
const maxDealPct = computed(() => deals.value.reduce((max, d) => Math.max(max, d.percent_off), 0))
...
function nudgeRail(dir: number) {
  ...
  el.scrollBy({
    left: dir * (rtl ? -1 : 1) * el.clientWidth * 0.7,
    behavior: 'smooth'
  })
}
</script>
    ...
    <section class="wrap">
      <ul class="trust">
        <li v-for="item in TRUST" :key="item.title" class="trust__cell">
          ...
          <span class="trust__meta"
            ><b>{{ t(item.title) }}</b
            ><i>{{
              item.sub === 'home.trust1_sub' ? t(item.sub, { n: freeShipOver }) : t(item.sub)
            }}</i></span
          >
        </li>
      </ul>
    ...
    <section v-if="categories.length" class="wrap block">
      <div class="railhead">
        ...
        <div class="rail__arrows">
          <button class="rail__btn" :aria-label="t('catalog.prev')" @click="nudgeRail(-1)">
            ‹
          </button>
...
.hero__scrim {
  ...
  background: linear-gradient(
    to var(--scrim-end, right),
    color-mix(in srgb, var(--hero-band) 92%, transparent) 0%,
    color-mix(in srgb, var(--hero-band) 62%, transparent) 48%,
    transparent 78%
  );
}
[dir='rtl'] .hero__scrim {
...
.hero__copy {
  ...
  gap: 12px;
  padding-block: clamp(1.25rem, 4.5vw, 3.5rem);
  padding-inline: max(60px, clamp(1.25rem, 4.5vw, 3.5rem));
...
.trust {
  ...
  padding: 14px clamp(0.75rem, 2vw, 1.5rem);
  display: grid;
...
[data-theme='dark'] .cat__imgwrap img {
...
.promo--b {
  background: var(--band);
  border-color: color-mix(in srgb, var(--border) 55%, var(--band));
}
.promo--campaign {
