...
const srcset = computed(() =>
  g.value ? `${g.value.preview_url} 600w, ${g.value.url} 900w` : undefined
)
...
<template>
  <main class="pdp">
    ...
    <div v-else-if="product" class="pdp__grid">
      ...
      <div class="pdp__info">
        <button class="pdp__back" @click="goBack">
          {{ t('common.back') }} {{ t('pdp.back') }}
        </button>
        <p class="eyebrow" v-if="product.category">
        ...
        <div class="pdp__pricing">
          <template v-if="deal && dealUnitPrice != null">
            ...
            <span class="pdp__price pdp__price--sale tnum">{{
              formatPrice(dealUnitPrice, product.currency)
            }}</span>
            <span class="pdp__dealchip">−{{ deal.percent_off }}%</span>
          </template>
          <span v-else class="pdp__price tnum">{{
        ...
        <label v-if="variants.length" class="pdp__field">
          ...
          <select v-model="selectedVariantId">
            <option v-for="v in variants" :key="v.id" :value="v.id" :disabled="v.stock <= 0">
              {{ v.name }} —
              {{ v.stock > 0 ? t('pdp.in_stock', { n: v.stock }) : t('pdp.sold_out') }}
            </option>
          </select>
        </label>
        ...
        >
          ...
          <p v-else-if="alertState === 'auth'" class="pdp__alert-note">
            <RouterLink to="/account">{{ t('pdp.sign_in') }}</RouterLink>
            {{ t('pdp.alert_signin_tail') }}
          </p>
          <button v-else class="pdp__alert-btn" @click="notifyMe">
            {{ t('pdp.notify_me') }}
          </button>
        </div>
        <ul class="pdp__meta">
    ...
    <div v-if="product" class="buybar">
      <div class="buybar__price">
        <template v-if="product.deal">
          <b class="tnum buybar__sale">{{
            formatPrice(dealUnitPrice ?? baseUnitPrice, product.currency)
          }}</b>
    ...
    <section v-if="product" class="reviews" :aria-label="t('pdp.reviews')">
      <h2>
        ...
        <span v-if="reviewData && reviewData.rating_count > 0" class="reviews__avg">
          {{ stars(reviewData.rating_avg ?? 0) }} {{ reviewData.rating_avg }} ·
          {{ reviewData.rating_count }}
        </span>
      </h2>
      ...
      <div v-if="reviewData?.mine" class="reviews__mine">
        {{ t('pdp.your_review_is') }}
        <strong>{{ t(`review.${reviewData.mine.status}` as MessageKey) }}</strong
      ...
      <form v-else class="reviews__form" @submit.prevent="submitReview">
        ...
        <label
          ><span>{{ t('pdp.title_optional') }}</span
          ><input v-model.trim="reviewForm.title" type="text"
        /></label>
        <label
          ><span>{{ t('pdp.your_review') }}</span
          ><textarea v-model.trim="reviewForm.body" rows="3" required />
        </label>
        <p v-if="reviewError" class="reviews__error" role="alert">
          {{ reviewError }}
        </p>
        <Button
          type="submit"
          :label="reviewBusy ? t('pdp.sending') : t('pdp.submit_review')"
          :disabled="reviewBusy || !reviewForm.body"
        />
      </form>
    </section>
  </main>
...
.pdp {
  ...
  margin: 0 auto;
  padding: var(--space-12) var(--container-pad) 0;
}
.pdp__grid,
.pdp__skeleton {
  display: grid;
...
[data-theme='dark'] .pdp__main img,
[data-theme='dark'] .pdp__thumb img {
...
.buybar {
  ...
  padding-bottom: calc(10px + env(safe-area-inset-bottom, 0px) * 0);
