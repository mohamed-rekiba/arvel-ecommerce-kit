  ...
  try {
    ...
    couponError.value =
      e instanceof ApiError
        ? (Object.values(e.errors)[0]?.[0] ?? t('cart.coupon_error'))
        : t('cart.coupon_error')
  }
}
...
<template>
  <main class="cart">
    ...
    <div v-if="state.loading" class="state" aria-busy="true">
      {{ t('common.loading') }}
    </div>
    ...
    <div v-else class="cart__grid">
        ...
        <li v-for="line in state.cart.items" :key="line.id" class="line">
          ...
          <div class="line__meta">
            ...
            <span class="line__unit">{{
              t('cart.each', { price: formatPrice(line.unit_price_cents) })
            }}</span>
            <button class="line__remove" @click="remove(line.id)">
              {{ t('cart.remove') }}
            </button>
          </div>
          <div class="qty" role="group" :aria-label="t('cart.quantity')">
            <button
              ...
              @click="update(line.id, Math.max(1, line.quantity - 1))"
            >
              −
            </button>
            <span>{{ line.quantity }}</span>
            <button :aria-label="t('cart.increase')" @click="update(line.id, line.quantity + 1)">
              +
            </button>
          </div>
      ...
      <aside class="summary">
        ...
        <form v-else class="coupon" @submit.prevent="submitCoupon">
          ...
          />
          <button class="btn" type="submit" :disabled="!couponCode">
            {{ t('cart.apply') }}
          </button>
        </form>
        <p v-if="couponError" class="coupon__error" role="alert">
          {{ couponError }}
        </p>

        <div class="summary__row summary__row--muted">
          <span>{{ t('cart.shipping') }}</span
          ><span>{{ t('cart.at_checkout') }}</span>
        </div>
        <div class="summary__row summary__row--total">
          <span>{{ t('cart.total') }}</span
          ><strong>{{ formatPrice(state.cart.total_cents - state.cart.discount_cents) }}</strong>
        </div>
        <button class="btn btn--primary summary__cta" @click="router.push('/checkout')">
          {{ t('cart.checkout') }}
        </button>
...
[data-theme='dark'] .line__thumb img {
