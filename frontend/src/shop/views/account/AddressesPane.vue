...
<template>
  <div class="card">
    <div class="head">
      ...
      <button v-if="!formOpen" class="cta cta--sm" @click="openAdd">
        + {{ t('account.addr_add') }}
      </button>
    </div>
    ...
    <form v-if="formOpen" class="form" @submit.prevent="save">
      <h3 class="form__title">
        {{ editing === null ? t('account.addr_add') : t('account.addr_edit') }}
      </h3>
      <label class="field">
        <span
          >{{ t('account.addr_label') }} <em>{{ t('common.optional') }}</em></span
        >
      ...
      <label class="field field--wide">
        <span
          >{{ t('account.addr_line2') }} <em>{{ t('common.optional') }}</em></span
        >
      ...
      <label class="field">
        <span
          >{{ t('account.phone') }} <em>{{ t('common.optional') }}</em></span
        >
      ...
      <div class="form__actions field--wide">
        ...
        <button class="ghost" type="button" @click="formOpen = false">
          {{ t('account.addr_cancel') }}
        </button>
      </div>
    </form>

    <p v-if="loading" class="muted">{{ t('common.loading') }}</p>
    <p v-else-if="!list.length && !formOpen" class="muted">
    ...
    <ul v-if="list.length" class="grid">
      <li v-for="a in list" :key="a.id" class="addr" :class="{ 'addr--default': a.is_default }">
        ...
        <p class="addr__body">
          {{ a.name }}<br />
          {{ a.line1 }}<template v-if="a.line2">, {{ a.line2 }}</template
          ><br />
          {{ a.city }}, {{ a.postal_code }} — {{ a.country }}
          <template v-if="a.phone"
            ><br /><span dir="ltr">{{ a.phone }}</span></template
          >
        </p>
        <div class="addr__actions">
          <button class="mini" @click="openEdit(a)">
            {{ t('account.addr_edit') }}
          </button>
          <button v-if="!a.is_default" class="mini" @click="setDefault(a)">
            {{ t('account.addr_set_default') }}
          </button>
          <button class="mini mini--danger" @click="remove(a)">
