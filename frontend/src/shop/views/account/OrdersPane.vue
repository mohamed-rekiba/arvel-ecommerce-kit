      ...
      <li v-for="o in orders" :key="o.id" class="order">
        <div class="order__head">
          <div class="order__facts">
            <span class="fact"
              ><i>{{ t('checkout.order') }}</i
              ><b>#{{ o.id }}</b></span
            >
            <span class="fact"
              ><i>{{ t('account.order_placed') }}</i
              ><b>{{ placedAt(o) }}</b></span
            >
            <span class="fact"
              ><i>{{ t('account.order_method') }}</i
              ><b>{{ t(`pay.${o.payment_method}` as MessageKey) }}</b></span
            >
            <span class="fact"
              ><i>{{ t('cart.total') }}</i
              ><b class="tnum">{{ formatPrice(o.total_cents) }}</b></span
            >
          </div>
          <span class="chip" :class="`chip--${o.status}`">{{
            t(`order.${o.status}` as MessageKey)
          }}</span>
        </div>
        ...
        <div class="order__body">
          ...
          <div class="order__actions">
            <RouterLink class="act act--primary" :to="`/orders/${o.id}`">{{
              t('account.order_track')
            }}</RouterLink>
            <button class="act" @click="openInvoice(o)">
            ...
            >
            <button v-if="cancellable(o)" class="act act--danger" @click="cancel(o)">
              {{ t('order.cancel') }}
            </button>
          </div>
        </div>
...
.act--danger {
  color: var(--sale);
  border-color: color-mix(in srgb, var(--sale) 40%, var(--border-2));
}
.act--danger:hover {
