  ...
  <div class="shop">
    ...
    <div class="tb">
      <div class="tb__in">
        <div class="tb__contact">
          <span class="tb__welcome">{{
            settings.get('store.welcome') || t('topbar.welcome')
          }}</span>
          <a :href="`mailto:${settings.get('store.email', 'shop@arvel.test')}`" class="tb__link">{{
            settings.get('store.email', 'shop@arvel.test')
          }}</a>
          <span class="tb__link tb__phone" dir="ltr">{{
            settings.get('store.phone', '+1 (086) 123-5678')
          }}</span>
        </div>
        <div class="tb__tools">
      ...
      <div class="nv__in">
        <div class="nv__links">
          ...
          <RouterLink
            :to="{ name: 'catalog' }"
            class="nv__a"
            :class="{ on: route.name === 'catalog' }"
            >{{ t('nav.shop') }}</RouterLink
          >
          <div ref="collRef" class="nv__dd">
            <button
              class="nv__a nv__dd-btn"
              :aria-expanded="collOpen"
              @click="collOpen = !collOpen"
              @keydown.escape="collOpen = false"
            >
    ...
    <div v-if="showAnnounce && announcement && !isDetail" class="ann">
      <p class="ann__txt">
        ...
        <template v-if="announcement.type === 'percent'">
          — {{ t('announce.percent', { n: announcement.value }) }}</template
        >
      </p>
    ...
    <footer class="ft">
      <div class="ft__top">
        ...
        <div class="ft__cols">
          <div>
            <h4>{{ t('footer.shop') }}</h4>
            <a>{{ t('footer.new_arrivals') }}</a
            ><a>{{ t('footer.audio') }}</a
            ><a>{{ t('footer.displays') }}</a
            ><a>{{ t('footer.accessories') }}</a>
          </div>
          <div>
            <h4>{{ t('footer.support') }}</h4>
            <a>{{ t('footer.track_order') }}</a
            ><a>{{ t('footer.shipping_returns') }}</a
...
.hd__in {
  ...
  grid-template-areas: 'menu brand . tools' 'search search search search';
  align-items: center;
...
.ann__code {
  ...
  font-weight: 800;
  letter-spacing: 0.04em;
  background: color-mix(in srgb, var(--on-accent-bright) 12%, transparent);
  padding: 1px 8px;
