...
const soldOut = computed(
  () => variants.value.length > 0 && variants.value.every((v) => v.stock <= 0)
)
...
const srcset = computed(() =>
  g.value
    ? `${g.value.thumb_url} 256w, ${g.value.preview_url} 600w, ${g.value.url} 900w`
    : undefined
)
  ...
  <article class="card" :class="{ 'card--out': soldOut }">
    ...
    <div class="info">
      ...
      <div
        ...
        class="stars"
        :aria-label="`${(product.rating_avg ?? 0).toFixed(1)} / 5`"
      >
      ...
      <div class="prices">
        <template v-if="deal">
          ...
          <span class="prices__now prices__now--sale tnum">{{
            formatPrice(deal.deal_price_cents, product.currency)
          }}</span>
        </template>
        <span v-else class="prices__now tnum">{{
