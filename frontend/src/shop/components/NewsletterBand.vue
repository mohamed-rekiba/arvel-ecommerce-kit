...
<template>
  <section class="nl">
    <div class="nl__in">
      ...
      <p v-if="state === 'done'" class="nl__msg nl__msg--ok" role="status">
        {{ t('newsletter.done') }}
      </p>
      <p v-else-if="state === 'error'" class="nl__msg nl__msg--err" role="alert">
        {{ t('newsletter.error') }}
      </p>
    </div>
  </section>
...
.nl__in {
  max-width: 1320px;
  margin: 0 auto;
  padding: clamp(1.75rem, 4vw, 2.75rem) clamp(1rem, 4vw, 2.5rem);
  display: grid;
...
.nl__form input {
  ...
  min-width: 0;
  padding: 12px 16px;
  border: 1px solid color-mix(in srgb, var(--nav-text) 30%, transparent);
  border-radius: var(--radius-full);
  background: color-mix(in srgb, var(--nav-text-hi) 8%, transparent);
  color: var(--nav-text-hi);
  font: inherit;
  font-size: 13.5px;
}
