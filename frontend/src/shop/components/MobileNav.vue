...
<template>
  <Transition name="mnav">
    <div v-if="open" class="mnav">
      ...
      >
        <RouterLink ref="firstLink" to="/" class="mnav__link" @click="emit('close')">{{
          t('nav.home')
        }}</RouterLink>
        <RouterLink :to="{ name: 'catalog' }" class="mnav__link" @click="emit('close')">{{
          t('nav.shop')
        }}</RouterLink>
        <button
          class="mnav__link mnav__acc"
          :aria-expanded="showCats"
          @click="showCats = !showCats"
        >
          {{ t('nav.collections') }}
          <span aria-hidden="true">{{ showCats ? '▴' : '▾' }}</span>
        </button>
        ...
        <RouterLink to="/deals" class="mnav__link mnav__link--deal" @click="emit('close')">{{
          t('nav.deals')
        }}</RouterLink>
...
.mnav-enter-active .mnav__panel,
.mnav-leave-active .mnav__panel {
...
.mnav-enter-from .mnav__panel,
.mnav-leave-to .mnav__panel {
  transform: translateX(-100%);
}
[dir='rtl'] .mnav-enter-from .mnav__panel,
[dir='rtl'] .mnav-leave-to .mnav__panel {
  transform: translateX(100%);
}
</style>
