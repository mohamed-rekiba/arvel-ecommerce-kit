...
export const api = {
  ...
  async addToCart(product_variant_id: number, quantity = 1) {
    return withCartToken(
      await request<Cart>('POST', `/cart/items`, {
        product_variant_id,
        quantity
      })
    )
  ...
  async order(id: number, orderToken?: string) {
    return request<Order>(
      ...
      `/orders/${id}`,
      undefined,
      orderToken ? { 'X-Order-Token': orderToken } : undefined
    )
  },
  ...
  async register(name: string, email: string, password: string) {
    const res = await request<{ token: string }>('POST', `/register`, {
      name,
      email,
      password
    })
  ...
  resetPassword(email: string, token: string, password: string) {
    return request<{ message: string }>('POST', `/reset-password`, {
      email,
      token,
      password
    })
  ...
  verifyEmail(id: number, token: string) {
    return request<{ message: string }>('POST', `/email/verify`, { id, token })
  },
  resendVerification() {
  ...
  changePassword(current_password: string, password: string) {
    return request<{ message: string }>('PUT', `/user/password`, {
      current_password,
      password
    })
  },
  ...
  uploadAvatar(file: File) {
    ...
    return apiFetch<Customer>(`/api/account/avatar`, {
      method: 'POST',
      body: form
    })
  },
