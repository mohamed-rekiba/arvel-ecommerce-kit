...
def client(tmp_path, monkeypatch):  # type: ignore[no-untyped-def]
    ...
    async def seed() -> None:
        ...
        cat = await Category.create(
            name={"en": "Phones", "fr": "Téléphones"}, slug="phones", published=True
        )
        await Product.create(
            name={"en": "Aurora Phone", "fr": "Téléphone Aurora"},
            slug="aurora",
            category_id=cat.id,
            vendor_id=vendor.id,
            description="d",
