...
def client(tmp_path, monkeypatch):
    ...
    async def migrate_and_seed() -> None:
        ...
        shirts = await Category.create(name="Shirts", slug="shirts")
...
def test_categories_index(client) -> None:
    ...
    assert names == ["Shirts", "Shoes"]  # ordered by name
...
def test_products_index_paginates_active_only(client) -> None:
    ...
    assert body["total"] == 25  # the draft is excluded by the default ACTIVE filter
...
def test_products_status_filter_can_surface_drafts(client) -> None:
    body = client.get("/api/products?status=draft").json()
    assert body["total"] == 1
    assert body["data"][0]["slug"] == "secret-draft"
